---
name: kb-ingest
description: Save a URL, file path, pasted text, DOI, or PMID into the knowledgebase at $KB_ROOT/raw/manual/YYYY/MM/DD/. Enriches via the relevant MCP (pubmed/crossref) when input is a DOI or PMID. NON-DESTRUCTIVE — write-only into raw/, never into pages/.
version: 0.1.0
author: dobry-ops
license: MIT
metadata:
...
Use Europe/Sofia date partitioning: `raw/manual/YYYY/MM/DD/`. Slug the date as
`YYYY_MM_DD`; slug the rest from input:

1. **URL** — `curl -fsSL --max-time 15 -o $KB_ROOT/raw/manual/YYYY/MM/DD/<date>_<slug>.<ext>`.
...
2. **File path** — copy with `install -m 0640` into `raw/manual/YYYY/MM/DD/`.
   Keep original extension. If `.pdf` and the title is not in the filename,
   leave it as is.
3. **Pasted text** — write to `raw/manual/YYYY/MM/DD/<date>_<slug>.md` with a
   single-line frontmatter (`source: pasted, accessed: <date>`).
4. **DOI** — call `crossref.crossref_lookup(doi)`. Save the full JSON to
   `raw/manual/YYYY/MM/DD/<date>_doi_<slug>.crossref.json`. If `crossref`
   returns a PMID, also call `pubmed.pubmed_fetch(pmid)` and save its result.
5. **PMID** — call `pubmed.pubmed_fetch(pmid)`. Save to
   `raw/manual/YYYY/MM/DD/<date>_pmid_<pmid>.pubmed.json`. If a DOI is in the
   response, ALSO call `crossref.crossref_lookup(doi)` and save that.
...
```
✓ Ingested: <kind> → kb/raw/manual/YYYY/MM/DD/<filename>
  Source:    <url|path|"pasted text">
  Enriched:  <yes/no — list the .json sidecar files if any>
  Next:      `research this` to extract findings into kb/content/<type>/YYYY/MM/<slug>.md
```
...
- Do NOT overwrite an existing `<date>_<slug>` file — append `_2`, `_3`
  as a disambiguator.
- If legacy raw files exist at `$KB_ROOT/raw/<date>_<slug>.*`, leave them alone
  unless `kb-maintain` is explicitly running; this ingest skill writes only to
