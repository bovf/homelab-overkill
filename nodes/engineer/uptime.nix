...
#
# The LAN IP lives encrypted in SOPS (nodes/engineer/ip) and resolves
# via placeholder when the uptime-kuma monitors template renders on the
# host — the repo is public, so it never appears here.
#
...
{config, ...}: {
  ...
  workloads.uptimeMonitors.engineer_ssh = {
    name = "Engineer SSH";
    type = "port";
    host = config.sops.placeholder."nodes/engineer/ip";
    port = 22;
    group = "Private";
    tags = ["host"];
  };
