                    ...
                    - |
                      ...
                      while true; do
                        ...
                        if [ "''${changed}" = "1" ] || [ ! -f /export/.published-head ]; then
                          ...
                          node -e 'const fs = require("fs"); const p = "/export/index.html"; const marker = "ms-kb-entrypoint"; const script = `<script id="ms-kb-entrypoint">(function () { var startHash = "#/page/Start%20Here"; var initialHash = window.location.hash || ""; var forceStartUntil = (initialHash === "" || initialHash === "#" || initialHash === "#/") ? Date.now() + 8000 : 0; function goStart(){ if (forceStartUntil && Date.now() < forceStartUntil && window.location.hash !== startHash) window.location.replace(window.location.pathname + window.location.search + startHash); } function fixBrowseLinks(){ document.querySelectorAll("a").forEach(function(a){ var t=(a.textContent||"").trim(); var h=a.getAttribute("href")||""; if (t === "[[/kb/]]" || t === "/kb/" || h.indexOf("%2Fkb%2F") !== -1 || h.indexOf("#/page//kb/") !== -1 || h === "#/page/kb") { a.href = "#/page/Index"; a.textContent = "KB Index"; a.target = "_self"; } }); } function addNav(){ var nav=document.querySelector(".cp__menubar-repos .nav-header"); if (!nav || document.getElementById("ms-kb-index-nav")) return; var d=document.createElement("div"); d.id="ms-kb-index-nav"; d.innerHTML="<a href=\"#/page/Index\" class=\"item group flex items-center text-sm font-medium rounded-md\"><span class=\"ui__icon ti ti-list-search\"></span><span class=\"flex-1\">KB Index</span></a><a href=\"/kb/\" class=\"item group flex items-center text-sm font-medium rounded-md\"><span class=\"ui__icon ti ti-folder\"></span><span class=\"flex-1\">Raw files</span></a>"; nav.appendChild(d); } function tick(){ goStart(); fixBrowseLinks(); addNav(); } tick(); document.addEventListener("DOMContentLoaded", tick); setInterval(tick, 250); }());</script>`; const html = fs.readFileSync(p, "utf8"); if (!html.includes(marker)) fs.writeFileSync(p, html.replace("</head>", script + "\n</head>"));'
