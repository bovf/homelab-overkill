                    ...
                    - |
                      ...
                      while true; do
                        ...
                        if [ "''${changed}" = "1" ] || [ ! -f /export/.published-head ]; then
                          ...
                          node -e 'const fs = require("fs"); const p = "/export/index.html"; const marker = "ms-kb-entrypoint"; const script = `<script id="ms-kb-entrypoint">(function () { function goStart(){ if (window.location.hash === "" || window.location.hash === "#" || window.location.hash === "#/") window.location.replace(window.location.pathname + window.location.search + "#/page/Start%20Here"); } function fixBrowseLinks(){ document.querySelectorAll("a").forEach(function(a){ var t=(a.textContent||"").trim(); var h=a.getAttribute("href")||""; if (t === "[[/kb/]]" || t === "/kb/" || h.indexOf("%2Fkb%2F") !== -1 || h.indexOf("#/page//kb/") !== -1) { a.href = "/kb/"; a.textContent = "Browse the whole KB"; a.target = "_self"; } }); } function addNav(){ var nav=document.querySelector(".cp__menubar-repos .nav-header"); if (!nav || document.getElementById("ms-kb-file-tree-nav")) return; var d=document.createElement("div"); d.id="ms-kb-file-tree-nav"; d.innerHTML="<a href=\"/kb/\" class=\"item group flex items-center text-sm font-medium rounded-md\"><span class=\"ui__icon ti ti-folder\"></span><span class=\"flex-1\">Browse KB files</span></a>"; nav.appendChild(d); } function tick(){ fixBrowseLinks(); addNav(); } goStart(); document.addEventListener("DOMContentLoaded", tick); setInterval(tick, 1000); }());</script>`; const html = fs.readFileSync(p, "utf8"); if (!html.includes(marker)) fs.writeFileSync(p, html.replace("</head>", script + "\n</head>"));'
