          ...
          containers = [{
            ...
            args = [''
              ...
              for i in $(seq 1 60); do
                if ${atticClient}/bin/attic login --set-default local http://attic-cache.proxy.svc.cluster.local:8090 "$ATTIC_ADMIN_TOKEN" >/tmp/attic-login.log 2>&1; then
                  break
                fi
              ...
              ${atticClient}/bin/attic login --set-default local http://attic-cache.proxy.svc.cluster.local:8090 "$ATTIC_ADMIN_TOKEN"
